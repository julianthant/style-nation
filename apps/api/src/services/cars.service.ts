...
import { SearchCarsDto, SortBy } from '../dto/cars/search-cars.dto';
...
export class CarsService {
  ...
  async findAll(searchDto: SearchCarsDto): Promise<{
    ...
    const where: Prisma.CarWhereInput = {
      ...
      ...(featured && {
        OR: [{ featured: true }, { featuredUntil: { gte: new Date() } }],
      }),
    };
  ...
  // Toggle featured boolean flag
  async toggleFeaturedBoolean(id: string, featured: boolean): Promise<CarEntity> {
    const car = await this.prisma.car.findUnique({
      where: { id },
    });
  ...
  // Set car as permanently featured
  async setFeaturedPermanent(id: string): Promise<CarEntity> {
    return this.toggleFeaturedBoolean(id, true);
  }

  // Remove permanent featured status
  async removeFeaturedPermanent(id: string): Promise<CarEntity> {
    return this.toggleFeaturedBoolean(id, false);
  }
